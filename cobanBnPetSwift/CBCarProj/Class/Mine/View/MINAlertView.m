...
- (void)createBackViewAndAlertView
{
    ...
    alertView.layer.cornerRadius = 3 * KFitWidthRate;
...
{
    _titleLabel = [MINUtils createLabelWithText:Localized(@"标题")size: 18 * KFitWidthRate alignment: NSTextAlignmentCenter textColor:kRGB(51, 51, 51)];
    ...
    _leftBottomBtn = [MINUtils createNoBorderBtnWithTitle:Localized(@"取消") titleColor: kRGB(96, 96, 96) fontSize: 15 * KFitWidthRate backgroundColor: kRGB(215, 215, 215)];
    ...
    [_leftBottomBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(alertView.mas_left).offset(10);
        make.bottom.mas_equalTo(alertView.mas_bottom).offset(-10);
    }];
    _rightBottomBtn = [MINUtils createNoBorderBtnWithTitle:Localized(@"确定") titleColor: [UIColor whiteColor] fontSize: 15 * KFitWidthRate backgroundColor: kBlueColor];
    [alertView addSubview: _rightBottomBtn];
    [_rightBottomBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo(45 * KFitHeightRate);
        make.left.equalTo(_leftBottomBtn.mas_right).offset(10);
        make.right.mas_equalTo(alertView.mas_right).offset(-10);
        make.bottom.mas_equalTo(alertView.mas_bottom).offset(-10);
    }];
}
...
- (void)showRightCloseBtn
{
    if (_rightCloseBtn == nil) {
        ...
        [closeImageView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(alertView).with.offset(16 * KFitHeightRate);
            make.right.equalTo(alertView).with.offset(-16 * KFitHeightRate);
            make.size.mas_equalTo(CGSizeMake(closeImage.size.width, closeImage.size.height));
        }];
        _rightCloseBtn = [[UIButton alloc] init];
        [alertView addSubview: _rightCloseBtn];
...
- (void)setContentViewHeight:(CGFloat)height
{
    ...
    [alertView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.height.mas_equalTo((100 + height) * KFitHeightRate);
    }];
