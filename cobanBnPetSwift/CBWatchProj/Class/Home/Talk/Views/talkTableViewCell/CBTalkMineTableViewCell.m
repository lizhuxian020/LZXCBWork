...
@interface CBTalkMineTableViewCell ()
/** 轮廓背景 */
...
@end
@implementation CBTalkMineTableViewCell

+  (instancetype)cellCopyTableView:(UITableView *)tableView {
    static NSString *cellID = @"CBTalkMineTableViewCell";
...
- (UIImageView *)avatarImgView {
    if (!_avatarImgView) {
        ...
        [_avatarImgView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.mas_equalTo(-20*frameSizeRate);
        }];
    }
...
- (UILabel *)nameLb {
    if (!_nameLb) {
        ...
        [_nameLb mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.width.mas_equalTo(80*frameSizeRate);
        }];
    }
    return _nameLb;
}
- (UIImageView *)bgmImgView {
    if (!_bgmImgView) {
        ...
        _bgmImgView.image = [UIImage imageNamed:@"chatto_bg_normal"];
        ...
        [_bgmImgView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.mas_equalTo(self.avatarImgView.mas_left).offset(-10*frameSizeRate);
        }];
    }
    return _bgmImgView;
}
- (UIButton *)playVoiceBtn {
    if (!_playVoiceBtn) {
        ...
        [self.bgmImgView addSubview:_playVoiceBtn];//chatto_voice_playing_f1
...
- (NSMutableArray *)imgArry {
    if (!_imgArry) {
        _imgArry = [NSMutableArray arrayWithObjects:@"爸爸", @"妈妈", @"姐姐", @"爷爷", @"奶奶", @"哥哥", @"外公", @"外婆", @"老师", @"自定义", @"校讯通", nil];
    }
    return _imgArry;
}
#pragma mark -- 播放语音
...
- (void)setTalkModel:(CBTalkModel *)talkModel {
    ...
    if (talkModel) {
        ...
        _timeLb.text = [CBWtUtils timeWithTimeIntervalString:talkModel.create_time];//talkModel.create_time;
        _nameLb.text = Localized(@"我");
        [_avatarImgView sd_setImageWithURL:[NSURL URLWithString:talkModel.head] placeholderImage:[UIImage imageNamed:self.imgArry[talkModel.type.integerValue]] options:SDWebImageLowPriority | SDWebImageRetryFailed | SDWebImageRefreshCached];
