//
//  CBPetFuncRightCvstionCell.swift
//  cobanBnPetSwift
//
...
class CBPetFuncRightCvstionCell: CBPetBaseTableViewCell {
    ...
    private lazy var msgLb:UILabel = {
        let lb = UILabel(text: "有时间吗？一起去遛狗？有时间吗？一起去遛狗？有时间吗？一起去遛狗？有时间吗？一起去遛狗？有时间吗？一起去遛狗？有时间吗？一起去遛狗？".localizedStr, textColor: UIColor.white, font: UIFont(name: CBPingFangSC_Regular, size: 14*KFitHeightRate)!,textAlignment:.left)
        lb.isHidden = true
        lb.numberOfLines = 0
        return lb
    }()
    private lazy var voiceTimeLb:UILabel = {
        let lb = UILabel(text: "20".localizedStr, textColor: UIColor.white, font: UIFont(name: CBPingFangSC_Regular, size: 14*KFitHeightRate)!)
        lb.isHidden = true
        return lb
    }()
    private lazy var playBtn:CBPetBaseButton = {
        let btn = CBPetBaseButton(imageName: "pet_func_chat_play")
        btn.setImage(UIImage(named: "pet_func_chat_stop"), for: .selected)
    ...
    override func layoutSubviews() {
        ...
        let maskPath = UIBezierPath.init(roundedRect: self.bgmView.bounds, byRoundingCorners: UIRectCorner(rawValue: UIRectCorner.topLeft.rawValue | UIRectCorner.bottomLeft.rawValue | UIRectCorner.bottomRight.rawValue), cornerRadii: CGSize(width: 18*KFitHeightRate, height: 18*KFitHeightRate))
    ...
    private func setupView() {
        ...
        self.bgmView.snp_makeConstraints { (make) in
            ...
            make.right.equalTo(self.avatarImageView.snp_left).offset(-8*KFitWidthRate)
        ...
        let playImage = UIImage(named: "pet_func_chat_play")!
        ...
        self.playBtn.snp_makeConstraints { (make) in
            ...
            make.size.equalTo(CGSize(width: playImage.size.width, height: playImage.size.height))
        }
    }
    var cvstionModel:CBPetFuncCvstionModel = CBPetFuncCvstionModel() {
        didSet {
            NotificationCenter.default.removeObserver(self)
            //self.cvstionModel.message_text = "234234dfggfsdfgsdfgwerfwerfwerfwergsdfgg四个dfgbdrt3453645sfbsdfsdf卢本伟"
            self.timeLb.text = CBPetUtils.convertTimestampToDateStr(timestamp: cvstionModel.add_time ?? "0", formateStr: "yyyy-MM-dd HH:mm:ss")
            self.avatarImageView.sd_setImage(with: URL.init(string: CBPetUserInfoModelTool.getUserInfo().photo ?? ""), placeholderImage: UIImage(named: "pet_psnal_defaultAvatar"), options: [])
            ...
            /* 1 已读*/
            //self.playBtn.isSelected = self.cvstionModel.isRead == "1" ? false : true
            ...
            if self.cvstionModel.message_text == nil || self.cvstionModel.message_text?.isEmpty == true {
                ...
                self.bgmView.snp_updateConstraints { (make) in
                    make.top.equalTo(self.avatarImageView.snp_top).offset(0*KFitHeightRate)
                    make.right.equalTo(self.avatarImageView.snp_left).offset(-8*KFitWidthRate)
                    make.bottom.equalTo(-15*KFitWidthRate)
                    make.width.equalTo(expectSize.width+24)
                    make.height.equalTo(expectSize.height)
                }
            } else {
                /* 文本*/
                self.showText(status: true) //SCREEN_WIDTH-128*KFitWidthRate-24*KFitWidthRate
                var expectSize = self.msgLb.sizeThatFits(CGSize(width: SCREEN_WIDTH-74*KFitWidthRate*2, height: SCREEN_HEIGHT))
