//
//  CBPetBottomSwitchBtnView.swift
//  cobanBnPetSwift
//
...
class CBPetBottomSwitchBtnView: CBPetBaseView {

    ///单例
    @objc public static let share:CBPetBottomSwitchBtnView = {
        let view = CBPetBottomSwitchBtnView.init()
        return view
    }()
    ...
    @objc public func showCtrlPanel(resultBlock:@escaping() -> Void) {
        UIApplication.shared.keyWindow?.subviews.forEach({ (vv) in
            if vv is CBPetBottomSwitchBtnView {
                return
            }
        })
        self.frame = CGRect(x: SCREEN_WIDTH-20*KFitWidthRate-ctrlPanelWidth, y: NavigationBarHeigt+12.5*KFitHeightRate+40*KFitHeightRate+15*KFitHeightRate+(15+46)*KFitHeightRate+50*KFitHeightRate, width: ctrlPanelWidth, height: ctrlPanelWidth)
        ...
        case "巴诺宠物".localizedStr:
            self.ctrPanelBtn.setImage(UIImage(named: "pet_switch_car_btn"), for: .normal)
            self.ctrPanelBtn.setImage(UIImage(named: "pet_switch_car_btn"), for: .selected)
            break
...
extension CBPetBottomSwitchBtnView {
    @objc private func switchClick() {
        ...
        case "巴诺宠物".localizedStr:
            ...
            sswitchModel.title = "巴诺车联网".localizedStr
