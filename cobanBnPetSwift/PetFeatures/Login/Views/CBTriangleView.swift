...
class CBTriangleView: UIView {
    ...
    private var Y: CGFloat {
       return self.frame.size.width * 0.5//minLength * 2 / 3
    }

    /// sin
    private let sinValue = sin(CGFloat.pi / 10)

    /// cos
    private let cosValue = cos(CGFloat.pi / 10)

    ///最外圈三角形
    private var outterTriangleLayer: CAShapeLayer = {
        let l = CAShapeLayer()
        l.strokeColor = UIColor.red.withAlphaComponent(0.5).cgColor
        l.fillColor = UIColor.orange.cgColor//UIColor.clear.cgColor
        l.opacity = 1
        l.lineWidth = 0//2
        return l
    }()
