...
//        self.confirmBtn.addTarget(self, action: #selector(regiseterClick), for: .touchUpInside)
...
//    }
    ...
    @objc private func getVerificationCodeClick() {
        ...
        if isGoogle() {
            CBPetNetworkingManager.share.getEmailCode(Email: self.phoneEmailView.textTF.text!, Type: "2") { [weak self] (successModel) in
...
//            var areaCode = self.areaCodeBTF.text!;
            ...
            CBPetNetworkingManager.share.getVerificationCode(Phone: self.phoneEmailView.textTF.text!, Type: "2",areaCode: areaCode, successBlock: { [weak self] (successModel) in
                if let value = self?.view {
                    MBProgressHUD.hide(for: value, animated: true)
                }
                /* 返回错误信息*/
    ...
    @objc private func retsetPwdRequestClick() {
        ...
        guard self.smsView.textTF.text!.count > 0 else {
...
//        var newCode = self.areaCodeBTF.text!
        ...
        let paramters:[String:String] = ["account":self.phoneEmailView.textTF.text!,"pwd":CBPetUtils.md5(Str: self.firPwdView.textTF.text!),"code":self.smsView.textTF.text!,"crCode":isGoogle() ? "" : newCode]
